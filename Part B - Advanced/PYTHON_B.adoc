...
implementation so that the two can vary independently. It provides a way to hide the
implementation details of a class from its clients. Typically the abstraction will define an
interface and the implementation will provide concrete classes that implement the interface. The
abstraction will then use an instance of the implementation to delegate the calls to.
