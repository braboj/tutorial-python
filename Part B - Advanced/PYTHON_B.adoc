...
the attributes and methods of the superclass and can add its own attributes and methods. The inheritance is the opposite process of abstraction and is used to represent an is-a relationship, e.g a student is a person.
...
Here's a short explanation of the key concepts and components of software architecture:
...
The deployment and infrastructure are the processes and tools that are used to develop, deploy and
run the software system. For example a web application may be developed using the Agile methodology
and deployed on a cloud infrastructure.

=== Components

The components are the building blocks of a software system. For example a web application may
have components for handling HTTP requests, authenticating users, and storing data in a database.
...
- Procedure calls (e.g. function call from one module to another)
- Shared memory (e.g. global variables in inter-process communication)
