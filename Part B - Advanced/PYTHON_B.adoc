...
Let's take an example where we are required to build a software that can handle packets for a
specific network protocol. A quick a dirty approach is to write a class that can handle all possible
packet types.

Of course we can do better and instead of creating a single class to handle all possible protocol
packet types you can create a separate class for each packet type. This makes it easier to
...
For example, you may have a class for each packet type that handles the packet serialization and
deserialization. Or you may have two classes, one for the packet serialization and one for the
packet deserialization.

In both cases, the classes have a single responsibility, but the responsibilities are different.
Choosing the right level of granularity for a class is a matter of experience and judgement.
...
In simple words the principle states that children shall not break by using the parents behavior.
This means that a subclass should be able to replace its parent class without breaking the code that
uses the parent class.

In a more formal way the derived classes must be substitutable for their base classes. Derived
classes only extend the functionality of base classes, they do not remove any functionality. A
method that behaves in a different way for a parent class and its child classes violates this
principle.

A very common example of violating this principle is when a subclass overrides a method of its
parent class to do nothing. Or when the parent class has to check the type of an object before
using it. This is a clear violation of the Liskov substitution principle.
...
- The parent has to check the type of the child before using it
- Override a method of a base class to do nothing
- Overriden methods change their behavior (e.g. different return type)
