...
Unfortunately the level of granularity is not always clear. For example, you may have a class that
handles both the parsing and building for a set of protocol packet types. Now the question is how
to split the class into several classes, each of which has a single responsibility.

One way to do his is to split the class into two classes, one for parsing and one for building.
Another way is to split the class into several classes, each of which handles a single packet
type. The best way to do this depends on the specific requirements of the application.
...
- Presentation layer (the frontend)
- Business layer (the backend)
...
The import system is a mechanism for loading modules and packages. It is used to import classes,
functions, and variables from one module into another module. The following are some of the most
common import statements:

- Every python file is a module
- Use `import` to import a module and have access to all the functions and classes in the module
- Use `from` to import a specific function or class from a module
- Each module lives in its own world called namespace
- A folder containing python file with `__init__`.py is a package
...
The namespaces are implemented as dictionaries in Python. The keys of the dictionary are the
identifiers, and the values are the objects. The built-in function `dir()` can be used to get a list
of all the identifiers in a namespace.

In a Python program, there are four types of namespaces:

- Built-In  (accessed using dir(__builtins__))
- Class     (accessed using dir(<class_name>))
...
The first thing that is executed when a module is imported is the module's code. The module's code
is executed in the module's namespace. The module's namespace is a dictionary that contains the
module's global variables. The module's namespace is created when the module is imported. The
module's namespace is destroyed when the module is unloaded.
