...
units. Modules can be imported into other modules using the `import` statement. The `import`
statement takes the name of the module to import as an argument.

// TODO: Introduction to namespaces or skip alltogether?
...
By creating a variable and assigning value to it, you create a key-value pair like in a dictionary.
The value is available under this name only in a specific region of the program called namespace.
Python runs multiple namespaces simultaneously, so variables with the same name from different namespaces do not
influence each other. The namespace defines the scope of the variable.
