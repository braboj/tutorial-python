...
Data classes are typically used when you want to create simple classes that mainly serve as data containers, and you
want to avoid writing boilerplate code for common methods like `\\__init__()`, `\\__repr__()`,
and `\\__eq__()`.
...
A real-world example of using the `Counter` class can be found in text analysis and natural language processing.
Let's say you're analyzing a large body of text, such as a collection of articles, tweets, or customer reviews,
and you want to extract insights about word frequency.
...
The `OrderedDict` is useful when you need to maintain the order of elements in a dictionary, which is
...
manipulated or skipped entirely based on a set criteria. In case of complicated manipulations using a for loop
is preferred as it easier to read.
...
which can be already existing dictionary. Optional separate conditional statements for key and value are possible.
Two expressions separated by colon in the beginning of the comprehension define the
key-value pair for the new dictionary.
